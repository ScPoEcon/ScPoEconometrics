...
```{r crime1,echo = FALSE,fig.cap = "Probability of arrest vs Crime rates in the cross section",message = FALSE}
css = crime4 %>% 
...
What's going on in figure \@ref(fig:anim)? 
...
The simplest approach is arguably this: we could take the equation literally and estimate a linear model where we include a dummy variable for each $i$. You can see in \@ref(eq:panel) that each $i$ has basically their own intercept $c_i$, so this works. Here is what we are talking about in a picture:
...
It's evident that *within* each county, there is a negative relationship. The dummy variable regression below will allow for different intercepts (county `1` will be the reference group), and one unique slope coefficient $\beta$:
