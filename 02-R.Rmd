...
OUT_PROMPT = "#OUT>"
knitr::opts_chunk$set(comment=OUT_PROMPT)
```
...
Typing one command after the other is not very convenient as our analysis becomes more involved. Ideally, we would like to collect all command statements in a file and run them one after the other, automatically. We can do this by writing so-called **scripts**, i.e. simple text files with extension `.R` or `.r` which can be *inserted* (or *sourced*) into an `R` session. RStudio makes this process very easy.
...
A quick note on styling choices in this book. We had to make a decision how to visually separate `R` code and resulting output in this book. We decided to prefix all output lines with ``r OUT_PROMPT`` to make the distinction. A typical code snippet with output is thus going to look like this:
...
where you see on the first line the `R` code, and on the second line the output. As mentioned, that line starts with ``r OUT_PROMPT`` to say *this is an output*, followed by `[1]` (indicating this is a vector of length *one* - more on this below!), followed by the actual result - `1 + 3 = 4`!
