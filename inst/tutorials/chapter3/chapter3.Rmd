...
set.seed(1)
x <- rlnorm(10000,meanlog=3.5,sdlog=1)
```
...
With continous distributions we cannot apply the grouping trick from the previous section. This is because it is extremely unlikely to ever observe two *identical* draws from such a distribution. Therefore, the only way to calculate the mean here is by computing $\overline{x}=(x_{1}+...+x_{N})/N$. The same is true for variance and SD.
...
2. compute the variance and SD of `x`!
...
Now suppose that `x` are measures of annual income obtained randomly from the population of London, and as such, are measured in Thousand Pound Sterling (GBP), i.e. `x=10` means *annual income of 10000GBP*. What is the *median* income in London? Remember that the median is the value above and below which exactly half of the data lie. Use the function `median` to compute this.
