...
output$dPlot <- renderPlot({
  ...
  if (input$dist == 'rnorm'){ # Normal
    range <- seq(-10, 10, .1)
    plot(range,
         dnorm(range, mean = input$mean_norm, sd = input$std_norm), "l", col = "navyblue", ylab = "Density", xlab = "y", main = "Normal Distribution")
    abline(v=input$mean_norm ,col="red",lw=3)
    abline(v=input$mean_norm ,col="blue",lw=1)
    ...
    plot(range,
        ...
        abline(v=exp(input$mean_lnorm) ,col="blue",lw=1)
        legend("topright",c("mean","median"),col=c("red","blue"),lty=c(1,1),lw=c(2,2))
    ...
    #abline(v=input$location * 2^(1/input$shape) ,col="blue",lw=1) Median of the Beta function -> no general closed-form...
    legend("topright",c("mean","median"),col=c("red","blue"),lty=c(1,1),lw=c(2,2))
    
  } else if(input$dist == 'rparet'){ #Pareto
    range <- seq(0, 10, .1)
    plot(range,
         dpareto(range, location = input$location, shape = input$shape), "l", col = "navyblue", ylab = "Density", xlab = "y", main = "Pareto Distribution")
    abline(v=(input$shape * input$location)/(input$shape -1),col="red",lw=3)
    abline(v=input$location * 2^(1/input$shape) ,col="blue",lw=1)
    legend("topright",c("mean","median"),col=c("red","blue"),lty=c(1,1),lw=c(2,2))
    
    
  } else if(input$dist == 'rarcsine'){ #Arcsine
    A <- distr::Arcsine()
    range <- seq(-.99, .99, 0.01)
    plot(range,
