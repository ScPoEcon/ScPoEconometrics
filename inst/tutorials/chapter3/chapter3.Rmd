...
output$dPlot <- renderPlot({
  ...
  } else if(input$dist == 'rarcsine'){
    A <- distr::Arcsine()
    range <- seq(-1, 1, 0.01)
    plot(range,
         d(A)(range), "l", col = "navyblue", ylab = "Density", xlab = "x", main = "Arcsine Distribution")
  } else if (input$dist == 'rationorm') {
    range <- seq(-5, 5, .1)
    plot(range,
         (input$a + dnorm(range, mean = 0, sd = 1)) / (input$b + dnorm(range, mean = 0, sd = 1)),
          "l", col = "navyblue", ylab = "Density", xlab = "x", main = "Ratio of 2 Normal Distributions")
  }
})
```
