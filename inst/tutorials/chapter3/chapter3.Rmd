...
The applet below allows you to explore the shape of the Binomial distribution for different values of $n$ and $p$. The red line is the (continuous) distribution that the Binomial distribution should converge to as we repeatedly draw many $X$s and increase the number of bins. As it turns out, this distribution is a *Normal distribution*.
...
```{r, echo=FALSE}
sliderInput("n2", "Number of draws n:", min = 1, max = 1000, step = 5, value = 41)
sliderInput("p2", "Probability of success p:", min = 0, max = 1, step = 0.01, value = .5)
#sliderInput("bins", "Number of bins:", min = 1, max = 50, step=5, value = 10)
plotOutput("binomPlot2")
```

```{r, context="server"}
output$binomPlot2 <- renderPlot({
  
  n <- input$n2
  p <- input$p2
  
  b <- dbinom(x = seq(0, n, 1), size = n, prob = p) # simulating the distrib by taking 10,000 draws
  
  barplot(height=b, names.arg = seq(0, n, 1),
          xlab = paste0("Number of Successes (out of ", n, " draws)."), 
          ylab = "Frequency", col="red",border="dodgerblue")
  #lines(seq(0, n, 1), dnorm(seq(0, n, 1), mean = n*p, sd = sqrt(n*p*(1-p))), col="red",lw=2, )
})
```
...
shinyUI(fluidPage(
  sidebarPanel(selectInput(inputId = "dist", # Dropdown Menu to choose a distribution
               ...
               conditionalPanel(condition = "input.dist == 'rarcsine'"),
               
               conditionalPanel(condition = "input.dist == 'rationorm'",
                                sliderInput("a", "Shift Numerator", min = -5, max = 5, step = .1, value = -2),
                                sliderInput("b", "Shift Denominator", min = -5, max = 5, step = .1, value = -3)),
...
output$dPlot <- renderPlot({
  ...
  if (input$dist == 'rnorm'){ # Normal
  ...
  } else if(input$dist == 'rarcsine'){ #Arcsine
    A <- distr::Arcsine()
    range <- seq(-1, 1, 0.01)
    plot(range,
    ...
    #abline(v=0, col="blue",lw=3)
    legend("topright","mean = median",col=c("red"),lty=1,lw=2)

    
  } else if (input$dist == 'rationorm') { # Ratio of 2 Normal Distributions (TODO: explain clearly what is being done.)
    range <- seq(-5, 5, .1)
    plot(range,
         (input$a + dnorm(range, mean = 0, sd = 1)) / (input$b + dnorm(range, mean = 0, sd = 1)),
          "l", col = "navyblue", ylab = "Density", xlab = "y", main = "Ratio of 2 Normal Distributions", 
         sub = "\nThis distribution is produced by the following formula: \n (shift_num + N(0, 1)) /  (shift_denom + N(0, 1))")
    abline(v=0, col="red",lw=3)
...
output$distPlot3 <- renderPlot({
  ...
  hist(nsample,col = 'darkgray', border = 'white',freq=FALSE,main=paste0("Histogram with N = ",input$ssize), xlim = c(0, 10))
  lines(density(nsample),col="red",lw=2)
  lines(seq(0, 10, .01), dnorm(x = seq(0, 10, .01), mean = 5, sd = 2), col = 'green')
