...
Luckily for us, fitting a linear model to some data does not require us to iteratively find the best intercept and slope manually. As it turns out, `R` can do this much more precisely in a matter of milliseconds.
...
$$(testscr)_i = \beta_0 + \beta_1 \times (str)_i + \epsilon_i$$
...
Again, we recognize our intercept and slope estimates from before, alongside some other cryptic numbers and indications. This output is called a *regression table*, and you will be able to decypher it by the end of this course.
...
As you probably expected, there is a slight downward correlation between a school's Student/Teacher Ratio and its average test results. Even though the data is very noisy, you computer was still able to pick up this trend. And using only 2 lines of code!
...
$$(testscr)_i = \beta_0 + \beta_1 \times (str)_i + \beta_2 (avginc)_i + \epsilon_i$$
