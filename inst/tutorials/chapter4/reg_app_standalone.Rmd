...
output$regPlot_dummy <- renderPlot({
  ...
  plot(x, y, type = "p", pch = 21, col = "blue", bg = "royalblue", asp=.25,
       ...
       cex = 1.2)
  if (input$meany_x0 == T){
    points(0, mean(dta[dta$x == 0, "y"]), col = 'orange', add = T, cex = 3, pch = 15)
    text(0, mean(dta[dta$x == 0, "y"]), "E[Y | X = 0]", pos = 4)
  }
  if (input$meany_x1 == T){
    points(1, mean(dta[dta$x == 1, "y"]), col = 'orange', add = T, cex = 3, pch = 15)
    text(1, mean(dta[dta$x == 1, "y"]), "E[Y | X = 1]", pos = 4)
  }
