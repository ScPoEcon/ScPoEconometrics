...
```{r, echo=FALSE}
shinyUI(fluidPage(
  sidebarPanel(sliderInput("i", "Intercept", min = -2,
                           max = 2, step = .5, value = 0),
...
output$regPlot <- renderPlot({
  ...
  coeffs <- c(-2, -1.5, -1, -.5, 0, .5, 1, 1.5, 2)
  a_true <- sample(coeffs, size = 1)
  b_true <- sample(coeffs, size = 1)
  ...
  legend("topleft", legend = paste0("r = ", round(cor(x, y), 3)))
