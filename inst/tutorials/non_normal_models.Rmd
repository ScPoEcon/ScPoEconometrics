...
plot(simulate_1_sample_normal(size = 10), xlim = c(-10, 15), ylim = c(-15, 35))
```
...
fit <- lm(y~x, simulate_1_sample_normal(size = 10))
summary(fit)
```
...
plot(simulate_1_sample_unif(size = 10), xlim = c(-10, 15), ylim = c(-15, 35))
```
...
fit <- lm(y~x, simulate_1_sample_unif(size = 10))
summary(fit)
```
...
se <- NULL
for (i in n){
  sim <- simulate_N_samples_unif(size = i, N = 1000)
...
curv <- function(x) sqrt(1.33/9)/sqrt(x)
...
curv = function(x) (1/sqrt(200)) * x

sig <- seq(0.2, 20, .5)
se = NULL
for (i in sig){
  x = rnorm(200, 5, 1)
  y = 7- 2*x + rnorm(200, 0, i) 
  fit <- lm(y~x, data.frame(x=x, y=y))
  se <- append(se, summary(fit)$coefficients[2,2])
}
