...
### Importing Excel Data (either in `.xlsx` or .`xls` formats)
...
```{r convert}
tot_pop$counts = as.numeric(tot_pop$counts)
tot_pop
```

Now you can see that column `counts` is indeed a `dbl`, i.e. a number with double precision, and we are fine.
