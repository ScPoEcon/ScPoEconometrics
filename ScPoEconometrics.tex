...
having chosen values \(b_0\) and \(b_1\):
...
Estimator}\label{ordinary-least-squares-ols-estimator}

The method to compute (or \emph{estimate}) \(b_0\) and \(b_1\) we
illustrated above is called \emph{Ordinary Least Squares}, or OLS.
\(b_0\) and \(b_1\) are also often called the \emph{OLS coefficients}.
By solving problem \eqref{eq:ols-min} one can derive an explicit formula
for them:
...
\subsection{Particular Cases of Linear
Regression}\label{particular-cases-of-linear-regression}

\subsubsection{No Regressor}\label{no-regressor}
...
\subsubsection{No Intercept}\label{no-intercept}
...
out both of those restrictions on our linear model by spending some with
...
\subsubsection{Centering A Regression}\label{centering-a-regression}
...
b_1 &= \frac{\frac{1}{N}\sum_{i=1}^N \tilde{x}_i \tilde{y}_i}{\frac{1}{N}\sum_{i=1}^N \tilde{x}_i^2}\\
    &= \frac{\frac{1}{N}\sum_{i=1}^N (x_i - \bar{x}) (y_i - \bar{y})}{\frac{1}{N}\sum_{i=1}^N (x_i - \bar{x})^2}  \label{eq:bline-centered}
\end{align}
...
all variables. So, unless all variables are centered, \textbf{always}
...
Let's check that these things are true in this sample of data:
...
\includegraphics{ScPoEconometrics_files/figure-latex/unnamed-chunk-128-1.pdf}
...
\[
y_i = b_0 + b_1 x_i + e_i \label{eq:ssr-mod} 
\]

and let us write down the benchmark model as follows:

\[
y_i = b_0 + e_i \label{eq:ssr-bench}
\]

As you can see, the benchmark model in \eqref{eq:ssr-bench} is a model
with an intercept only. We just saw that this delivers an estimate of
the mean of \(y\). It is a benchmark because it does not include
\emph{any} explanatory variables, so we can compare against this other
models which do in fact contain some \(x\)'s. Back to our \(R^2\)
statistic: there are several equivalent definitions, and for our present
...
\(y_i = b_0 + b_1 x_i + e_i\). The right panel shows the SSR of
\(y_i = b_0 + e_i\). Ideally, each red square would be small relative to
its blue counterpart, indicating that our model has a small residual at
a given observation.
...
\item
  ...
  \texttt{income\ \textasciitilde{}\ age}. You can also ask \texttt{R}
  to estimate a multivariate regression such as
  \(income = b_0 + b_1 \cdot age + b_2 \cdot isWoman\) by simply
  separating all variables on the right-hand side of the equation with
  the \texttt{+} operator, like this :
...
\begin{center}\includegraphics{ScPoEconometrics_files/figure-latex/unnamed-chunk-131-1} \end{center}
...
\(\beta_1\). We will learn more about it in chapter \ref{std-errors}.
...
\(\hat{\beta}_0\), \(\hat{\beta}_1\) and \(\hat{\beta}_2\) are
\emph{estimates} of some unkown \emph{population parameters}, there is
some degree of \textbf{uncertainty} about their values. An other way to
say this is that we want some indication about the \emph{precision} of
those estimates.
...
\begin{center}\includegraphics{ScPoEconometrics_files/figure-latex/unnamed-chunk-137-1} \end{center}
...
\(\beta_1\) do \textbf{not} have a ``hat'', which is widely used to
indicate an estimate. Now, the fact that our data \((y_i,x_i)\) are a
sample from a larger population means that there will be \emph{sampling
variation} in our estimates \(\hat{\beta}_0\) and \(\hat{\beta}_1\) -
exactly like in the case of the sample mean estimating the population
average as mentioned above. In general, the more observations we have
...
\item
  ...
  linear combination of other variables. For example, suppose we have
  two variables for each individual, \(x_1,x_2\) which determine an
  outcome \(y\). If \(x_{2} = 2x_{1}\), then \(x_2\) provides no new
  information. It's enough to know \(x_1\), and multiply it by 2. In
  this sense, \(x_2\) is a redundant variable and should not be included
  in the model.
\item
...
Under assumptions 1. through 4. we can define the formula for the
standard error of our slope coefficient in the context of our single
regressor model \eqref{eq:abline-5} as follows:

\[
Var(\beta_1|x_i) = \frac{\sigma^2}{x_1^2 + x_2^2 + \dots + x_n^2}  \label{eq:var-ols}
\] In pratice, we don't know the theoretical variance of
\(\varepsilon\), but we form an estimate about it from our sample of
...
\[
Var(\beta_1|x_i) = \frac{SSR}{(n-p)(x_1^2 + x_2^2 + \dots + x_n^2)}  \label{eq:var-ols2}
\]
...
is used to measures the impact of number of bathrooms (\(x\)) on the
sales price of houses (\(y\)). You find a positive impact of bathrooms
on houses:
...
\(E[\varepsilon] = 0\) is \emph{very} close to zero (type
...
term \(\varepsilon\) which is \emph{correlated} with \texttt{bathrms}.
Going back to our discussion about \emph{ceteris paribus} in section
\ref{ceteris}, we stated that the interpretation of our OLS slope
estimate is that
...
Well that's the problem then. We said above that one more bathroom is
worth 27477 dollars - if \textbf{nothing else changes}! But that doesn't
seem to hold, because we have seen that as we increase \texttt{bathrms}
from \texttt{1} to \texttt{2}, the mean of the resulting residuals
changes quite a bit. So there \textbf{is something in \(\varepsilon\)
which does change}, hence, our conclusion that one more bathroom is
worth 27477 dollars is in fact \emph{invalid}!
...
\emph{decreased} from 27477 to 22811.5 by almost 5000 dollars! The way
in which \texttt{bathrms} and \texttt{lotsize} are correlated is
important here, so let's investigate that:

\begin{figure}

{\centering \includegraphics{ScPoEconometrics_files/figure-latex/unnamed-chunk-143-1} 

}

\caption{Distribution of `lotsize` by `bathrms`}\label{fig:unnamed-chunk-143}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dta}\OperatorTok{$}\NormalTok{is.male =}\StringTok{ }\KeywordTok{factor}\NormalTok{(x)  }\CommentTok{# convert x to factor}
\NormalTok{dummy_reg =}\StringTok{ }\KeywordTok{lm}\NormalTok{(y}\OperatorTok{~}\NormalTok{is.male,dta)}
\end{Highlighting}
\end{Shaded}
