...
better tool for statistical analysis. It even generated this book! There
are many good resources for learning \texttt{R}.
...
interface to \texttt{R} which allows you to efficiently.

The best way to appreciate the value of RStudio is to start using R
\emph{without} RStudio. To do this, double-click on the R GUI that you
should have downloaded on your computer following the steps above (on
windows), or start R in your terminal (on Linux or Mac) by just typing
\texttt{R} in a terminal, see figure \ref{fig:console}. You've just
opened the R \textbf{console} which allows you to start typing code
right after the \texttt{\textgreater{}} sign, called \emph{prompt}. Try
typing \texttt{2\ +\ 2} or \texttt{print("Your\ Name")} and hit the
return key. And \emph{voilà}, your first R commands!
...
\textbf{scripts}, and RStudio makes this process very easy.

Open RStudio by clicking on the RStudio application on your computer,
and notice how different the whole environment is from the basic R
console -- in fact, that very same \texttt{R} console is running in your
bottom left panel. The upper-left panel is a space for you to write
...
\item
  ...
  \emph{understands}
\item
  \emph{script}: a list of commands, each separated by a new line
\end{itemize}
...
introduces the \emph{pipe operator} \texttt{\%\textgreater{}\%} from the
\texttt{magrittr} package.\footnote{A \emph{pipe} is a concept from the
  Unix world, where it means to take the output of some command, and
  pass it on to another command. This way, one can construct a
  \emph{pipeline} of commands. For additional info on the pipe operator
  in R, you might be interested
...
\begin{center}\includegraphics{ScPoEconometrics_files/figure-latex/unnamed-chunk-122-1} \end{center}
