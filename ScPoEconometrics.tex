...
illustrate the issues at stake a question from epidemiologie and public
health instead:
...
debated, and it's instructive for us to look at this history. This
...
only (hence, \emph{directed} graph). Here is a simple example of a DAG,
in which \(x\) \textbf{causes} \(y\):

\begin{verbatim}
#OUT> 
#OUT> Attaching package: 'ggdag'
\end{verbatim}

\begin{verbatim}
#OUT> The following object is masked from 'package:ggplot2':
#OUT> 
#OUT>     expand_scale
\end{verbatim}

\begin{verbatim}
#OUT> The following object is masked from 'package:stats':
#OUT> 
#OUT>     filter
\end{verbatim}
...
\emph{smoking} was the only relevant difference between a population of
smokers and one of non-smokers. In particular, they worried that people
\textbf{self-selected} into smoking, and that the choice to become a
smoker may be influenced by other, unobserved, underlying forces - like
genetic predisposition, for example. That could mean that smokers were
also more likely to take risks, or more likely to be heavy drinkers, or
engage in other behaviours that might be conducive to develop lung
cancer. They did not formulate it in terms of genetics at the time,
because they could not know until the 2000's, when the human genome was
sufficiently mapped to establish this fact (and indeed there \textbf{is}
a smoking gene! But that's beside the point), but they worried about
this factor.
...
are we to tell what would have happened? It's hard, but it's
fascinating. We \emph{imagine} what would have happened.
...
\emph{one} of both outcomes, we face \textbf{fundamental identification
...
To learn about the true \(\delta^{ATE}\), it matters who ends up being
treated!

Now imagine that individuals have at least some partial knowledge about
...
impact of \(x\) on \(y\). Writing down this equation tightly linked to
drawing this DAG from above:
