...
Assumptions 1. and 3. are mainly of a technical nature. Assumption 2., however, is key for our understanding of the regression machinery in econometrics. Imagine model \@ref(eq:abline-5) measures the sales price of houses, and that the explanatory variable $x$ measures the number of rooms in each house sold. You find a positive impact of 

Example 

```{r}
data(Housing, package="Ecdat")
ggplot(data=subset(Housing,lotsize<13000 & bathrms<4),aes(x=lotsize,color=factor(bathrms),fill=factor(bathrms))) + geom_density(alpha=0.2)
```
