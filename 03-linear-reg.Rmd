...
```{r wooldridge-wages, echo=TRUE,fig.cap='Wages vs Education from the wooldridge dataset \\emph{wage1}.',fig.height=7}
...
rug(wage1$wage, side=2, col="red")

# plot 2
hist(wage1$wage,prob = TRUE, col = "grey", border = "red", 
     main = "Histogram of wages and Density")
...
The interpretation of this estimation is that

1. With zero year of education, the hourly wage is about -0.9 dollars per hour
1. Each additional year of education increase hourly wage by an additional 54 cents.
...
Suppose we wanted to use the above estimates to report the effect of years of education on wages, with the only difference that we now measure wages not in terms of hours, but in terms of *years*, i.e. we want to know the effect on *annual* wage. Let's assume we have full-time workers, 7h per day, 5 days per week, 45 weeks per year. Calling this factor $\delta = 7 \times 5 \times 45 = 1575$, we have that $x$ dollars per hour imply $x \times \delta = x \times `r 7*5*45`$ dollars per year. 

What would be the effect of using $wage \times `r 7*5*45`$ instead of $wage$ as outcome variable on our regression coefficients $b_0$ and $b_1$? Well, let's try!
...
Let's call the coefficients in the column labelled (1) as $b_0$ and $b_1$, and let's call the ones in column (2) $b_0^*$ and $b_1^*$. In column (1) we see that another year increaeses hourly wage by `r round(c1[2],2)` dollars, as before.  In column (2), the corresponding number is `r round(c2[2],2)`, i.e. another year of education will increase *annual* wages by `r round(c2[2],2)` on average. Notice however, that $b_0 \times \delta = `r round(c1[1],2)` \times `r delta` = `r round(c2[1],2)` = b_0^*$ and that $b_1 \times \delta = `r round(c1[2],2)` \times `r delta` = `r round(c2[2],2)` = b_1^*$, that is we just had to multiply both coefficients by the scaling factor applied to original outcome $y$! Also, observe that the $R^2$s of both regressions are identical!

So, we see that rescaling the outcome variable $y$ is relatively straightforward. Just multiply all coefficients by the scaling factor, and you retain the identically same fit to the data. 

Rescaling the regressors $x$ is slightly different, but it's easy to work out *how* different, given the linear nature of the covariance operator, which constitutes the OLS estimator. Suppose we rescale $x$ by the number $c$. Then, using the OLS formula in \@ref(eq:beta1hat), we see that we get new slope coefficient $b_1^*$ via
...
That is, we change the slope by the *inverse* of the scaling factor applied to regressor $x$, but the intercept is unaffected from this. You should play around for a while with our rescaling app:
...
1. Several important economic variables (like wages, city size, firm size, etc) are approximately *log-normally* distributed. By transforming them with the $\log$, we obtain an approzimately *normally* distributed variable, which has desirable properties for our regression.
1. Applying $\log$ reduces the impact of outliers.
...
We Let's use the `update` function to modify the previous regression:

```{r}
log_hourly_wage = update(hourly_wage, log(wage) ~ ., data = wage1)
```

The `update` function takes an existing `lm` object, like `hourly_wage` here, and updates the `formula`. Here the `.` on the right hand side means *leave unchanged*. Let's compare the output between both models:
...
```{block type = "note"}
We call a regression of the form $\log(y) = b_0 + b_1 x + u$ a *log-level* specification, because only the right hand side has been transformed. Here, the impact of increasing $x$ by one unit is to increase $y$ by $100 \times b_1$ **percent**. In our example: an additional year of education will increase hourly wages (and **not** log (!) wages) by 8.3%.
```

Notice that the $R^2$ slightly improved, so have a better fit to the data. Whether we apply the $log$ to left or right-hand side makes a difference, as outlined in this table:
...
this is in table \@ref(tab:loglog)
